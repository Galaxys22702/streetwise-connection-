...
function usageSummary(order) {
  ...
  return {
    ...
    usedBytes,
    remainingBytes,
    percentUsed: limitBytes
      ? Number(Math.min(100, (usedBytes / limitBytes) * 100).toFixed(2))
      : null
  };
}
...
async function insertOrder(order) {
  ...
  await query(
    ...
    [
      ...
      order.currency,
      order.providerMode,
      order.liveOrderExecuted,
      order.install,
      order.error,
      order.dataLimitBytes,
      order.dataUsedBytes,
      order.activatedAt,
      order.expiresAt,
      order.createdAt,
...
async function findOrderByIdempotencyKey(userId, key) {
  ...
  if (!databaseConfigured) {
    return [...memoryOrders.values()].find(
      (order) => order.userId === userId && order.idempotencyKey === key
    ) || null;
  }
...
export async function listProviderBundles({ country } = {}) {
  return getProvider().listBundles({ country });
}

export async function checkProviderCoverage(input = {}) {
  return getProvider().checkCoverage(input);
}
