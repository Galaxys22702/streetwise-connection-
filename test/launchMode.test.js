...
test("identifies customer and diagnostic routes for waitlist blocking", () => {
  assert.equal(isCustomerServicePath("/api/auth/register"), true);
  assert.equal(isCustomerServicePath("/api/payments/checkout"), true);
  assert.equal(isCustomerServicePath("/api/payments/status"), true);
  ...
  assert.equal(isCustomerServicePath("/api/provider/status"), true);
