...
export function selectMergedBranches(branches, pullRequests, repository, defaultBranch) {
  return branches.flatMap(branch => {
    ...
    const merged = ownPullRequests.find(pr =>
      ...
      pr.base?.repo?.full_name === repository && pr.base.ref === defaultBranch &&
      pr.head.sha === branch.commit.sha
    );
    return merged ? [{ name: branch.name, sha: branch.commit.sha, pullRequest: merged.number }] : [];
  });
}
...
async function main() {
  ...
  const [branches, pullRequests] = await Promise.all([list("branches"), list("pulls?state=all")]);
  const candidates = selectMergedBranches(branches, pullRequests, repository, metadata.default_branch);
  console.log(`${apply ? "Cleanup" : "Preview"}: ${candidates.length} unchanged merged branches qualify.`);
  let deleted = 0;
  for (const branch of candidates) {
    console.log(JSON.stringify(branch));
    if (!apply) continue;
    const current = await api(`branches/${encodeURIComponent(branch.name)}`);
    const open = await api(`pulls?state=open&head=${encodeURIComponent(`${metadata.owner.login}:${branch.name}`)}&per_page=1`);
  ...
  const summary = `${apply ? `Deleted ${deleted}` : `Previewed ${candidates.length}`} unchanged branches whose pull requests were merged into ${metadata.default_branch}.\n`;
  console.log(summary);
