# Streetwise Connection LLC — Signature-Ready Owner Packet

Prepared: 2026-08-30
Status: PREPARED FOR OWNER COMPLETION AND SIGNATURE

This packet consolidates the non-sensitive facts already established for Streetwise Connection. It is designed so the owner can transfer the information into the official Nevada, IRS, local licensing, tax, banking, and provider forms when those forms are presented. It is not itself a government-issued form or licence.

## 1. Company identity

- Proposed legal name: **Streetwise Connection LLC**
- Trade/public name: **Streetwise Connection**
- Entity type: **Nevada limited liability company**
- Ownership at formation: **Robert Tursi — 100%**
- Management: **Member-managed**
- Responsible party: **Robert Tursi**
- State: **Nevada**
- Principal locality: **Las Vegas / Clark County, Nevada, subject to final address-jurisdiction confirmation**
- Accounting year end: **December**, unless changed on professional tax advice

## 2. Business purpose / activity description

**Short form**

Branded residential and commercial cellular connectivity services using approved licensed wholesale network providers, with customer account servicing, compatibility guidance, SIM/eSIM setup, line management, security-focused support, and related connectivity assistance.

**Long form**

Streetwise Connection provides technology-enabled connectivity services for residential and commercial customers. The company plans to resell or facilitate approved cellular connectivity obtained through contracted licensed wholesale telecommunications providers. Planned capabilities may include mobile data, hotspot, voice, SMS, local phone numbers, number portability, SIM/eSIM activation, roaming, account-security controls, customer support, device setup guidance, compatibility checks, and related line-management services where supported by the final provider agreement. The company will not sell live telecommunications service until required provider agreements, licences, registrations, and regulatory responsibilities are confirmed.

## 3. Nevada formation transfer sheet

Use the official Nevada Secretary of State / SilverFlume filing interface. Transfer the following prepared facts:

- Entity: Limited Liability Company
- Name: Streetwise Connection LLC, subject to state acceptance
- Management: Member-managed
- Initial member/manager: Robert Tursi
- Effective date: Date of filing unless owner chooses another permitted date
- Initial List: File with formation package
- State Business Licence: Obtain with formation package unless an actual exemption applies

Owner must privately enter and verify:

- Registered agent legal name
- Registered office address
- Principal/business address
- Mailing address
- Organizer details if requested
- Identity verification
- Electronic certifications/signatures
- Filing payment

Owner certification/signature: ______________________________

Printed name: **Robert Tursi**

Date: ______________________________

## 4. IRS EIN transfer sheet

Complete only after Nevada accepts the LLC.

- Legal name: Streetwise Connection LLC
- Trade name: Streetwise Connection
- Entity: LLC
- Members: 1
- State of formation: Nevada
- Responsible party: Robert Tursi
- Reason: Started a new business
- Principal activity: Telecommunications / cellular connectivity services
- Description: Branded cellular connectivity reseller and customer account/support platform using contracted licensed wholesale providers; planned service may include data, voice, SMS, phone-number features and SIM/eSIM activation subject to provider and regulatory approval
- Accounting year end: December unless changed on tax advice

Owner privately supplies SSN/ITIN, addresses, formation date, and any employee/payroll answers. Do not store those values in this public repository.

Owner certification/signature if required by the filing method: ______________________________

Date: ______________________________

## 5. Nevada tax/local licensing transfer sheet

After EIN and Nevada Business ID are issued:

- Legal name: Streetwise Connection LLC
- Trade name: Streetwise Connection
- Entity: LLC
- Owner/responsible principal: Robert Tursi
- Business activity: Use the long-form description above
- Customer markets: Residential and commercial
- Nevada Business ID: [ENTER PRIVATELY WHEN ISSUED]
- EIN status: [ENTER STATUS WHEN ISSUED; DO NOT COMMIT EIN]
- Physical location/jurisdiction: [OWNER TO VERIFY]
- Home-based: [OWNER TO CONFIRM FROM ACTUAL OPERATING LOCATION]
- Required tax accounts: [SELECT ONLY AFTER ACTUAL TAX RESPONSIBILITY IS CONFIRMED]

Owner signature: ______________________________

Date: ______________________________

## 6. Provider commercial onboarding declaration

Streetwise Connection LLC seeks an AT&T-approved reseller/wholesale path as its primary domestic provider strategy, with 1GLOBAL retained as fallback. The intended service includes recurring U.S. cellular connectivity for residential and small-business customers, subject to written approval. The customer experience may include plan selection, compatibility guidance, SIM/eSIM provisioning, voice, SMS, phone-number features, hotspot, account support, security-focused controls and roaming where the selected provider agreement permits them.

Requested written provider confirmations before launch:

1. Contractual role of Streetwise Connection LLC.
2. Provider of record to the end customer.
3. Permission for recurring U.S. domestic use.
4. Permission for residential resale/use.
5. Permission for commercial resale/use.
6. Permitted data allowances, throttling, hotspot/tethering, and fair-use terms.
7. Wholesale pricing and billing rules.
8. Refund, failed activation, fraud, SLA, and escalation responsibilities.
9. Telecom tax/surcharge responsibility.
10. FCC/USAC and Nevada regulatory responsibility allocation.
11. Permission to market the provider-backed plans under the Streetwise Connection brand.
12. For AT&T: approved program path, Tier 1 support model, end-user billing model, FRN requirement/status, API access and branding/co-branding rights.

Streetwise target retail structure, subject to provider approval and economics:

- Streetwise Home: $25/month
- Business Starter: $20/month per line
- Business Volume: $15/month per line for 3+ lines
- Business Pro: $30/month per line

For Streetwise Connection LLC:

Authorised signer: **Robert Tursi**

Title: **Sole Member / Owner**

Signature: ______________________________

Date: ______________________________

Provider legal entity: ______________________________

Provider authorised signer: ______________________________

Title: ______________________________

Signature: ______________________________

Date: ______________________________

AT&T-specific transfer and signature fields are prepared separately in `docs/business/AT&T-SIGNATURE-READY-PACKET.md`.

## 7. Internal launch authorisation

This section is signed only after formation, licensing, provider approval, regulatory responsibility, plan economics, production security, payment testing, and one controlled provisioning test are complete.

I, Robert Tursi, as authorised owner of Streetwise Connection LLC, approve enabling commercial production only after the repository launch-readiness checks report that all mandatory gates have passed.

Owner signature: ______________________________

Date: ______________________________

Approved production date: ______________________________

## 8. Items intentionally not prefilled in GitHub

The following must remain private and should be entered only into official portals or secure company records:

- SSN/ITIN
- full residential/private address
- government ID numbers or images
- bank/routing/account numbers
- EIN number itself unless a private operational system specifically requires it
- provider API keys/secrets
- payment processor secrets
- signatures after execution

## 9. Implementation handoff

After documents are issued/signed, update the backend compliance/provider records with non-sensitive status, issuer, effective date, expiry/renewal date, contractual role, approved rights, and approved plan mappings. Keep original signed documents in secure private company storage, not the public source repository.
